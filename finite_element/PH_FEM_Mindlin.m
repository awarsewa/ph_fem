...
classdef PH_FEM_Mindlin < PH_FEM_element
    ...
    methods(Access = public)
        function obj = PH_FEM_Mindlin(order, corner_nodes, rho, E, nu, h, kappa)   
            
            [n_, J_, Q_, G_, nodes_xy] = pH_Mindlin_PFEM_p(order, corner_nodes, rho, E, nu, h, kappa);
            ...
            obj.name = 'PH-FEM Mindlin plate element';
            ...
            for n=1:obj.n_nodes
                ...
                obj.nodes{n} = PH_MechanicalNode([n obj.n_nodes+n 2*obj.n_nodes+n], 1, [nodes_xy(n,:) 0]);
            end
            ...
            obj.elements{1} = PH_Element('PH_FEM_Disk', 1:obj.n_nodes, 1:obj.n_nodes, ...
                                        ...
                                        'kappa', kappa); 
            % Ports
            obj.n_ports = 3*obj.n_nodes;
            
            for n = 1:obj.n_nodes
                %PH_MechanicalPort_boundary(type, nodes, orientation, element, IOPair, inputName, outputName)
                obj.ports{n} = PH_MechanicalPort_boundary('force', n, [0; 0; 1], 1, n, ['F' num2str(n) '_z'], ['v' num2str(n) '_z']);
                obj.ports{obj.n_nodes+n} = PH_MechanicalPort_boundary('torque', n, [1; 0; 0], 1, obj.n_nodes + n, ['M' num2str(n) '_x'], ['w' num2str(n) '_x']);
                obj.ports{2*obj.n_nodes+n} = PH_MechanicalPort_boundary('torque', n, [0; 1; 0], 1, 2*obj.n_nodes + n, ['M' num2str(n) '_y'], ['w' num2str(n) '_y']);
            end
        end
    ...
    methods(Access = protected)
        ...
        function cp = copyElement(obj)
            % PH_FEM_Mindlin(order, corner_nodes, rho, E, nu, h, kappa)   
            cp = PH_FEM_Mindlin(obj.order, obj.corner_nodes, obj.density, obj.youngsModulus, obj.poissonRatio, ...
                                    obj.thickness, obj.timoshenkoShearCoefficient);
            obj.copyData(cp);
        end
    end
end