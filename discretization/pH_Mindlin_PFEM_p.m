...
Q = [1/(rho*h)*inv(M), zeros(N, 7*N); ...
    zeros(N), 12/(h^3*rho)*inv(M), zeros(N, 6*N); ...
    zeros(N, 2*N) 12/(h^3*rho)*inv(M), zeros(N, 5*N); ...
    zeros(N, 3*N), E*h^3/(12*(1-nu^2))*inv(M), nu*E*h^3/(12*(1-nu^2))*inv(M), zeros(N, 3*N); ...
    zeros(N, 3*N), nu*E*h^3/(12*(1-nu^2))*inv(M), E*h^3/(12*(1-nu^2))*inv(M), zeros(N, 3*N); ...
    zeros(N, 5*N), E*h^3/(24*(1+nu))*inv(M), zeros(N, 2*N); ...
    zeros(N, 6*N), kappa*Gs*h*inv(M), zeros(N); ... 
    zeros(N, 7*N), kappa*Gs*h*inv(M)];
