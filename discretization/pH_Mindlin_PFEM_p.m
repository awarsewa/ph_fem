function [n, J, Q, G, nodes_xy] = pH_Mindlin_PFEM_p(order, nodes_xy, rho, E, nu, h, kappa)
...
J = [zeros(N, 6*N), -D_x, -D_y; ... 
     zeros(N, 3*N), -D_x, zeros(N), -D_y, M, zeros(N); ...
     zeros(N, 4*N), -D_y, -D_x, zeros(N), M; ...
     zeros(N), D_x' zeros(N, 6*N); ...
     zeros(N, 2*N), D_y', zeros(N, 5*N); ...
     zeros(N), D_y', D_x', zeros(N, 5*N);
     D_x', -M', zeros(N, 6*N); ...
     D_y', zeros(N), -M', zeros(N, 5*N)];
 
Q = [M\(eye(N).*(1/(rho*h))), zeros(N, 7*N); ...
    zeros(N), M\(eye(N).*(12/(h^3*rho))), zeros(N, 6*N); ...
    zeros(N, 2*N) M\(eye(N).*(12/(h^3*rho))), zeros(N, 5*N); ...
    zeros(N, 3*N), M\(eye(N).*(E*h^3/(12*(1-nu^2)))), M\(eye(N).*nu*E*h^3/(12*(1-nu^2))), zeros(N, 3*N); ...
    zeros(N, 3*N), M\(eye(N).*nu*E*h^3/(12*(1-nu^2))), M\(eye(N).*E*h^3/(12*(1-nu^2))), zeros(N, 3*N); ...
    zeros(N, 5*N), M\(eye(N).*E*h^3/(24*(1+nu))), zeros(N, 2*N); ...
    zeros(N, 6*N), M\(eye(N).*kappa*Gs*h), zeros(N); ... 
    zeros(N, 7*N), M\(eye(N).*kappa*Gs*h)];
